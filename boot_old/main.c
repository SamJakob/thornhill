...
short unsigned int* memoryTypes[] = {
...
struct ThornhillSystemTime readTime() {
    ...
    while (isRTCUpdateInProgress()) {}
    if (THCompareSystemTime(time, _performTimeRead())) return time;
...
EFI_STATUS efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
...

    {
        ...
        while (offset < endOfMemoryMap) {
            ...
            Print(L"%u %s: %u (%u pages)\r\n", descriptor->Type, memoryTypes[descriptor->Type], descriptor->PhysicalStart,
    ...
    // return;
    // SystemTable->BootServices->Stall(1000000 * 5);
